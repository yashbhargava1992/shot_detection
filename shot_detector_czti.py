...
gap_start = mf.gap_detector(czti_time_band1,10)
gap_end = gap_start+1
print gap_start
...
#~ jump = len(czti_time_band1)/246
#~ for i in range(0,len(czti_time_band1),jump):
for i,seg_end in enumerate(gap_start):

	#~ if i+jump<=len(czti_time_band1):short_seg = np.arange(i,i+jump,1)
	#~ else: short_seg = np.arange(i,len(czti_time_band1),1)
	if i==0: short_seg = np.arange(0,seg_end,1)
	elif i==len(gap_start)-1: 
		short_seg = np.arange(gap_end[i],len(czti_time_band1),1)
	else: short_seg = np.arange(gap_end[i-1],seg_end,1)
	T=32
	if seg_end+T>=len(czti_time_band1): continue
	#~ print 'short_seg', short_seg
	# Plotting shots in Band 1
	ax1 = plt.subplot(211)
	ax1.errorbar(czti_time_band1[short_seg], czti_rate_band1[short_seg], yerr=czti_r_er_band1[short_seg],alpha=0.5,color='grey')
	#~ plt.plot(czti_time_band1[short_seg], czti_rate_band1[short_seg])
	#~ peak_pos = mf.peak_detector(czti_time_band1[short_seg], czti_rate_band1[short_seg], czti_r_er_band1[short_seg],f=1,T=2,shot_sep=-1,small_peak_flag=False)
	peak_pos,other_peaks = mf.peak_detector(czti_time_band1[short_seg], czti_rate_band1[short_seg], czti_r_er_band1[short_seg],f=3,T=32,shot_sep=0.5,small_peak_flag=True,sig_det='std')
	#~ ax1.plot(czti_time_band1[short_seg][other_peaks], czti_rate_band1[short_seg][other_peaks],'or',label='Other peaks')
	ax1.plot(czti_time_band1[short_seg][peak_pos], czti_rate_band1[short_seg][peak_pos],'og',label='SHOT')
	#~ ax1.errorbar(back_czti_time_band1[short_seg], back_czti_rate_band1[short_seg], yerr=back_czti_r_er_band1[short_seg])
	ax1.set_ylabel('Count rate czti')
	plt.legend()
	#~ print czti_time_band1[short_seg][0], unit2_time_band1[short_seg][0]
	...
	#~ rat_err,rat = mf.error_propagator_for_ratio(czti_rate_band2[short_seg],czti_r_er_band2[short_seg],czti_rate_band1[short_seg],czti_r_er_band1[short_seg],True)
	#~ ax3.plot(czti_time_band2[short_seg], czti_rate_band2[short_seg]/czti_rate_band1[short_seg])
	#~ ax3.errorbar(czti_time_band2[short_seg], rat,yerr=rat_err,alpha=0.5)
	#~ ax3.plot(czti_time_band2[short_seg][other_peaks], rat[other_peaks],'o',label='Other peaks')
	#~ ax3.plot(czti_time_band2[short_seg][peak_pos], rat[peak_pos],'o',label='SHOT')
	...
	list_of_peak_times = np.append(list_of_peak_times,czti_time_band1[short_seg][peak_pos])
	
	#~ plt.show()
	plt.clf()
#~ print list_of_peak_indices
np.savetxt('czti_3sig_index_list_0p05_czti_fullrange.txt',list_of_peak_indices,fmt='%1i')		# Saving the list of indices to be used with 0.05 s LC of the observation. 
np.savetxt('czti_3sig_peak_time_list_0p05_czti.txt',list_of_peak_times)
